import HomepageSection from "../../models/HomepageSection.js";
import Product from "../../models/Product.js";
import { ApiError } from "../../utils/ApiError.js";

const PRODUCT_CARD_FIELDS =
  "title slug basePrice offerPrice specialPrice oldPrice baseImage images ratingsAverage isHotSale isNewArrival isFlashSale stock hasVariants";

const resolveProducts = async (section) => {
  if (section.sourceType === "category") {
    if (!section.category) return [];
    return Product.find({ category: section.category, isActive: true })
      .sort({ createdAt: -1 })
      .limit(section.limit)
      .select(PRODUCT_CARD_FIELDS);
  }

  if (section.sourceType === "promo") {
    if (!section.promoFlag) return [];
    return Product.find({ [section.promoFlag]: true, isActive: true })
      .sort({ createdAt: -1 })
      .limit(section.limit)
      .select(PRODUCT_CARD_FIELDS);
  }

  if (section.sourceType === "custom") {
    if (!section.products?.length) return [];
    const products = await Product.find({
      _id: { $in: section.products },
      isActive: true,
    }).select(PRODUCT_CARD_FIELDS);
    const byId = new Map(products.map((p) => [String(p._id), p]));
    return section.products.map((id) => byId.get(String(id))).filter(Boolean);
  }

  return [];
};

// Public: returns active sections, each with its products/categories (or
// nothing, for "promo_banner") already resolved, ready for the homepage to
// render directly.
export const getActiveSectionsWithProducts = async () => {
  const sections = await HomepageSection.find({ isActive: true })
    .populate("category", "name slug")
    .populate("categories", "name slug banner")
    .sort({ sortOrder: 1 });

  const resolved = await Promise.all(
    sections.map(async (section) => ({
      _id: section._id,
      title: section.title,
      subtitle: section.subtitle,
      sourceType: section.sourceType,
      layout: section.layout,
      banner: section.banner,
      promoBanner: section.promoBanner,
      category: section.category,
      categories: section.categories,
      products:
        section.layout === "grid" || section.layout === "featured"
          ? await resolveProducts(section)
          : [],
    }))
  );

  // Don't show empty sections (e.g. a category with nothing active in it
  // yet). "categories" is judged by categories.length, "promo_banner" has
  // no data dependency at all — it's shown whenever it's active, since a
  // heading + description is enough on its own.
  return resolved.filter((s) => {
    if (s.layout === "categories") return s.categories?.length > 0;
    if (s.layout === "promo_banner") return true;
    return s.products.length > 0;
  });
};

// Admin: raw section configs (not resolved) for the editor screen.
export const getAllSections = () =>
  HomepageSection.find()
    .populate("category", "name slug")
    .populate("products", "title slug baseImage")
    .populate("categories", "name slug banner")
    .sort({ sortOrder: 1 });

export const createSection = async (data) => {
  validateSourceFields(data);
  return HomepageSection.create(data);
};

export const updateSection = async (id, data) => {
  if (data.sourceType || data.layout) validateSourceFields(data);
  const section = await HomepageSection.findByIdAndUpdate(id, data, {
    new: true,
    runValidators: true,
  });
  if (!section) throw new ApiError(404, "Homepage section not found.");
  return section;
};

export const deleteSection = async (id) => {
  const section = await HomepageSection.findByIdAndDelete(id);
  if (!section) throw new ApiError(404, "Homepage section not found.");
};

const validateSourceFields = (data) => {
  // "promo_banner" needs at minimum a heading — everything else (image,
  // description, CTA) is optional. It never touches sourceType/products.
  if (data.layout === "promo_banner") {
    if (!data.promoBanner?.heading) {
      throw new ApiError(400, "Add a heading for the Promo Banner section.");
    }
    return;
  }

  // "categories" layout skips the product-source validation entirely — it
  // uses its own `categories` array instead.
  if (data.layout === "categories") {
    if (!data.categories || data.categories.length === 0) {
      throw new ApiError(
        400,
        "Select at least one category for a Category Showcase section."
      );
    }
    return;
  }

  if (!data.sourceType) {
    throw new ApiError(400, "Select where this section's products come from.");
  }
  if (data.sourceType === "category" && !data.category) {
    throw new ApiError(400, "Select a category for a category-based section.");
  }
  if (data.sourceType === "promo" && !data.promoFlag) {
    throw new ApiError(
      400,
      "Select a promo flag (Featured, New Arrival, Hot Sale, or Flash Sale)."
    );
  }
  if (
    data.sourceType === "custom" &&
    (!data.products || data.products.length === 0)
  ) {
    throw new ApiError(400, "Add at least one product for a custom section.");
  }
  if (data.layout === "featured" && !data.banner?.image?.url) {
    throw new ApiError(
      400,
      "Upload a banner image for a Featured Collection section."
    );
  }
};
